...
class GuessAI extends Component {
  ...
  setAnswer(n) {
    const answerRecord = this.state.answerRecord;
  ...
  addGuess(newGuess) {
    ...
    if (this.guessPerRound[this.state.entireRound]['featuresChosenByExplainer'].length === 0) {
      for (let i = 0; i < 4; i++) {
        const curImage = this.state.hintVis[i];
        const imgIdx = curImage ? curImage.substring(curImage.lastIndexOf('/') + 1, curImage.indexOf('.')) : '-1';
  ...
  setScoreImages(n) {
    ...
    const images = this.state.scoreImages;
    images.push(n);
