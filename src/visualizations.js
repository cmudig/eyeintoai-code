...
class Vis extends Component {
    ...
    renderVis() {
        ...
        for (let i = 0; i < 10; i++) {
            element1.push(
                <div key={"vis" + i}>
                    <div class="order" id={"order" + i}>{this.returnIndex(imglist[i])}</div>
                    <img src={imglist[i]} className="active" alt={this.answer + " visualization " + (i + 1)} key={this.answer + " visualization " + (i + 1)} onClick={(ev) => { this.selectVis(ev.target.classList, imglist[i], i); (ev.target.classList.contains("selected")) ? ev.target.classList.remove("selected") : ev.target.classList.add("selected") }} />
                </div>)
        }
    ...
    render() {
        ...
        return (
            <div className="modal-select" >
                <img src={this.props.image} id="answerImg" />
            ...
            of the {this.props.answer} for hints</div>
